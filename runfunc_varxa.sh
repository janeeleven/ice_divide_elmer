...
while read -r a0 DivPos filnam remainder; do

mkdir -p icerise_run
#touch Korff_run/examplefile.txt
mkdir -p icerise_run/mesh/
mkdir -p icerise_run/src/
bash update_sif_vardiv.sh ${a0} ${DivPos} ${filnam} #create new sif file from template
##cp Case_*.sif output/
...
gsutil cp icerise_run/mesh/*.vtu ${outputbucket}run_${a0}_${DivPos}_varaccum/

echo [`date +"%D %T"`] Finished Case_${a0}_${DivPos}_varaccum | tee -a log.txt
done < ${inputfil}
